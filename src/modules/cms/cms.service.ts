...
import { ICms, IFooterCms } from './entities/cms.entity';
import { CMS_TYPE } from './enums/cms.enum';
import { IHomeCms } from './entities/homeCms.entity';
import { IAboutCms } from './entities/aboutCms.entity';
import { IProductCms } from './entities/productCms.entity';
import { IContactUs } from './entities/contactUs.entity';
...
export class CmsService {
  ...
  async updatePage(pageName: CMS_TYPE, body: any): Promise<any> {
    ...
    switch (pageName) {
      case CMS_TYPE.HOME:
        ...
        break;
      case CMS_TYPE.ABOUT_US:
        doc = await this.updateAboutPage(body);
        break;
      case CMS_TYPE.CONTACT_US:
        doc = await this.updateContactUsPage(body);
        break;
      case CMS_TYPE.PRODUCT:
        doc = await this.updateProductPage(body);
        break;
  ...
  async updateAboutPage(body: IAboutCms) {
    const cms = await this.Cms.findOne({ pageName: CMS_TYPE.ABOUT_US }).lean();
    const prev = cms?.[CMS_TYPE.ABOUT_US];

    const doc = await this.Cms.findOneAndUpdate(
      { pageName: CMS_TYPE.ABOUT_US },
      { [CMS_TYPE.ABOUT_US]: body },
      {
        new: true,
        upsert: true,
      },
  ...
  async updateContactUsPage(body: IContactUs) {
    const cms = await this.Cms.findOne({
      pageName: CMS_TYPE.CONTACT_US,
    }).lean();
    const prev = cms?.[CMS_TYPE.CONTACT_US];

    const doc = await this.Cms.findOneAndUpdate(
      { pageName: CMS_TYPE.CONTACT_US },
      { [CMS_TYPE.CONTACT_US]: body },
      {
        new: true,
        upsert: true,
      },
  ...
  async updateProductPage(body: IProductCms) {
    const cms = await this.Cms.findOne({ pageName: CMS_TYPE.PRODUCT }).lean();
    const prev = cms?.[CMS_TYPE.PRODUCT];

    const doc = await this.Cms.findOneAndUpdate(
      { pageName: CMS_TYPE.PRODUCT },
      { [CMS_TYPE.PRODUCT]: body },
      {
        new: true,
        upsert: true,
      },
  ...
  async updateFooterPage(body: IFooterCms) {
    const cms = await this.Cms.findOne({ pageName: CMS_TYPE.FOOTER }).lean();
    const prev = cms?.[CMS_TYPE.FOOTER];

    const doc = await this.Cms.findOneAndUpdate(
      { pageName: CMS_TYPE.FOOTER },
      { [CMS_TYPE.FOOTER]: body },
      {
        new: true,
        upsert: true,
      },
